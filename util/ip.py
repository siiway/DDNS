...
try:
    # python2
    from urllib2 import urlopen, Request # type: ignore -> disable vscode tip
except ImportError:
    # python3
...
def public_v4(url="https://getip.wyf9.top/", reg=IPV4_REG):  # 公网IPV4地址
    allowed_gai_family_backup = urllib3_cn.allowed_gai_family # backup
    def allowed_gai_family():
        return AF_INET # Force ipv4
    urllib3_cn.allowed_gai_family = allowed_gai_family # change
    ret = _open(url, reg)
    urllib3_cn.allowed_gai_family = allowed_gai_family_backup # restore
    return ret


def public_v6(url="https://getip.wyf9.top/", reg=IPV6_REG):  # 公网IPV6地址
    allowed_gai_family_backup = urllib3_cn.allowed_gai_family # backup
    def allowed_gai_family():
        return AF_INET6 # Force ipv6
    urllib3_cn.allowed_gai_family = allowed_gai_family # change
    ret = _open(url, reg)
    urllib3_cn.allowed_gai_family = allowed_gai_family_backup # restore
    return ret
