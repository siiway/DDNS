...
where_default = 'https://1.1.1.1/dns-query'


def get(qname, rtype, where=where_default) -> list:
...
def get_v4(qname):
    resp = str(get(qname, rtype=dns.rdatatype.A)[0])
    return compile(IPV4_REG).search(resp).group()


def get_v6(qname):
    resp = str(get(qname, rtype=dns.rdatatype.AAAA)[0])
    return compile(IPV6_REG).search(resp).group()
